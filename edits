...
def getCoordsOnLand():
    ...
    url = "ne_10m_admin_0_countries.zip"
    world = gpd.read_file(url).explode()
    land = world.dissolve().geometry.iloc[0]
    ...
    N = totalStars * 30  # oversample so we can filter effectively
    ...
    rndn_sample = gpd.GeoDataFrame(
        rndn_sample,
        geometry=gpd.points_from_xy(x=rndn_sample.x, y=rndn_sample.y),
        crs="EPSG:4326"
    )

    # Filter points that are on land (not in the ocean)
    inLand = rndn_sample.geometry.within(land)
    global points
    points = rndn_sample.loc[inLand, "geometry"]

    # Downsample to exactly totalStars points
    if len(points) > totalStars:
        points = points.sample(totalStars)
    
    print(f"{len(points)} coordinates generated on land (out of {N} random points)")
